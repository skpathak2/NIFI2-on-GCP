...
resource "google_compute_instance" "nifi" {
  ...
  metadata_startup_script =   <<EOF
        ...
        then 
          ...
          gsutil cp ${var.nifi-bucket}/binaries/nifi-toolkit-${var.nifi-toolkit-version}-bin.zip ${var.nifi-path}
          unzip ${var.nifi-path}/nifi-toolkit-${var.nifi-toolkit-version}-bin.zip -d ${var.nifi-path}
          rm ${var.nifi-path}/nifi-toolkit-${var.nifi-toolkit-version}-bin.zip
          ...
          su nifi -c 'export PATH=$PATH:/usr/lib/jvm/jdk/bin && cd ${var.nifi-path}/nifi-${var.nifi-version}/conf && ${var.nifi-path}/nifi-toolkit-${var.nifi-toolkit-version}/bin/tls-toolkit.sh client  -c ${var.nifi-ca-hostname} -t ${var.ca-token} '
          sleep 1
          done
